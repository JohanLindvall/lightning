...
// buffer themselves (skipBlocksTakesTail) — the AVX2 and NEON ones as the
// buffer's last 64 bytes shifted by the lanes before the cursor, the AVX-512
// one with a masked load of exactly the remainder — so the close landing at
// every distance from the END of the buffer is a separate case of each, and so
// is every cursor position the shift is computed from.
