...
func DecodeFloat64SliceArena(out *[]float64, data []byte, i int, a *Arena[float64]) (int, error) {
...
func decodeFloat64Slice(out *[]float64, data []byte, i int, a *Arena[float64]) (int, error) {
	...
	for first := true; ; first = false {
		...
		// An element that starts with a digit or a '-' is where the SIMD
		// decimal-run kernel takes over (useFloatRun): it writes every following
		// element of at most 15 digits and no exponent straight into the
		// slice's spare capacity — each one Clinger's exact fast path, so the
		// value scanFloat and strconv give — and hands back at the first element
...
func DecodeIntSliceArena[T intKind](out *[]T, data []byte, i int, a *Arena[T]) (int, error) {
...
func decodeIntSlice[T intKind](out *[]T, data []byte, i int, a *Arena[T]) (int, error) {
	if uint(i) >= uint(len(data)) {
		return i, ErrTruncated
	}
	if data[i] == 'n' {
...
// (see DecodeFloat64SliceArena); the //lightning:arena decoders call it.
func DecodeUintSliceArena[T uintKind](out *[]T, data []byte, i int, a *Arena[T]) (int, error) {
	return decodeUintSlice(out, data, i, a)
}

// decodeUintSlice is the shared body; a nil arena presizes with make, exactly
// as before the arena existed (see decodeFloat64Slice).
func decodeUintSlice[T uintKind](out *[]T, data []byte, i int, a *Arena[T]) (int, error) {
	if uint(i) >= uint(len(data)) {
		return i, ErrTruncated
	}
	if data[i] == 'n' {
...
func DecodeFloat64Array(out []float64, data []byte, i int) (int, error) {
	...
	// A coordinate point ([2]float64, [3]float64) is the whole array in one
	// call of the SIMD decimal-run kernel, closed at its ']': when it fills
	// every slot and closes the array, that is the answer, and the clear and
	// the loop below — a memclr call per point, 3.7% of canada once the
	// conversions got cheap — are not needed. Anything else (a shorter or
	// longer array, an element the kernel hands back) starts again below,
	// where the kernel's partial writes are cleared.
	//
	// Only the kernel's VBMI body is used here: coordinates are 16- and
	...
	for first := true; ; first = false {
		...
		if run && !hold && idx < len(out) && (data[i]-'0' <= 9 || data[i] == '-') {
			n, p, closed := parseFloatRunV(data, i, out[idx:])
			if n > 0 {
				idx += n
				i = p
				if closed != 0 {
				...
				hold = true // the element it stopped at is the scalar code's
				continue
			}
			run = false
		}
...
// zeroing to save.
func DecodeByteSliceArena(out *[]byte, data []byte, i int, a *Arena[byte]) (int, error) {
	return decodeByteSlice(out, data, i, a)
}

// decodeByteSlice is the shared body (see decodeFloat64Slice for the pattern).
func decodeByteSlice(out *[]byte, data []byte, i int, a *Arena[byte]) (int, error) {
	if uint(i) >= uint(len(data)) {
		return i, ErrTruncated
	}
	if data[i] != '"' {
