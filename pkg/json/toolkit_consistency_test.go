...
func TestStripDefaultsKeepKeyHonorsWhitespaceMode(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "after_colon_container", in: `{"b":  { "x":0 }}`, want: `{"b":{"x":0}}`, ws: RemoveWhitespace},
		...
		{name: "nested_containers", in: `{"b":{ "x" : [ 0 , { "y" : 0 } ] }}`, want: `{"b":{"x":[0,{"y":0}]}}`, ws: RemoveWhitespace},
		{name: "kept_between_siblings", in: `{"a": 1 , "b": { "x" : 0 } , "c": 2}`, want: `{"a":1,"b":{"x":0},"c":2}`, ws: RemoveWhitespace},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			// In place must equal fresh, with no exceptions: the kept member is
			// re-emitted from the snapshot when the output aliases the input, so
			// compacting reads bytes the walk cannot have overwritten. Three of
			// these cases could not be checked here while the snapshot and the
			// compaction lived on separate branches; they can now, and they are
			// the ones that would break first if either half regressed.
			inplace := []byte(c.in)
