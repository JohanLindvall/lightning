...
// returns a non-nil error, iteration stops and that error is returned — unless
// it is [ErrStop], which ends the walk with a nil result. Non-target members
// along the path are skipped without allocating.
...
// returns a non-nil error, iteration stops and that error is returned — unless
// it is [ErrStop], which ends the walk with a nil result. Non-target members
// along the path are skipped without allocating. [ArrayEachIndex] is the same
// walk with the element's index handed to fn.
