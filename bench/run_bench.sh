...
for dir in */; do
	...
	# An any-root case (type Benchmark any) measures decoding the whole document
	# into Go's generic interface{} value. That cannot be a generated method —
	# Go forbids methods on an interface type — so there is nothing to generate
	# and no easyjson; lightning is exercised through its dynamic entry point
	# support.DecodeValue, and the reflection libraries decode into a plain `any`.
	if grep -qE '^type Benchmark (any|interface\{\})$' "${dir}data.go"; then
...
import (
	...
	"github.com/JohanLindvall/lightning/pkg/support"
...
// BenchmarkLightning measures support.DecodeValue, lightning's dynamic decoder —
// the path taken for any-typed fields — reading the document into an any value.
func BenchmarkLightning(b *testing.B) {
	b.SetBytes(int64(len(benchInput)))
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		if _, _, err := support.DecodeValue(benchInput, support.SkipWS(benchInput, 0)); err != nil {
			b.Fatal(err)
		}
	}
}

// BenchmarkStdlib measures encoding/json decoding into interface{}.
...
// BenchmarkSonic measures bytedance/sonic decoding into interface{}.
...
// BenchmarkSonicFastest is BenchmarkSonic with sonic's ConfigFastest.
...
// BenchmarkGoccy measures goccy/go-json decoding into interface{}.
...
// BenchmarkJSONV2 measures json/v2 decoding into interface{}.
